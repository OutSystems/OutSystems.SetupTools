...
InModuleScope -ModuleName OutSystems.SetupTools {
    Describe 'Install-OSServer Tests' {
        ...
        Context 'When user is not admin' {
            ...
            $result = Install-OSServer -Version '11.23.0.0' -ErrorVariable err -ErrorAction SilentlyContinue
            ...
            It 'Should not throw' { { Install-OSServer -Version '11.23.0.0' -ErrorAction SilentlyContinue } | Should Not throw }
        }

        Context 'When the platform server is installed with a lower version' {

            Mock GetServerVersion { return '11.23.0.0' }
            $result = Install-OSServer -Version '11.23.1.0' -ErrorVariable err -ErrorAction SilentlyContinue
            ...
            It 'Should not throw' { { Install-OSServer -Version '11.23.1.0' -ErrorAction SilentlyContinue } | Should Not throw }
        }

        Context 'When the platform server is installed with a higher version' {

            Mock GetServerVersion { return '11.23.0.2' }
            $result = Install-OSServer -Version '11.23.0.1' -ErrorVariable err -ErrorAction SilentlyContinue
            ...
            It 'Should return the right result' {
                ...
                $result.RebootNeeded | Should Be $false
                $result.ExitCode | Should Be -1
                $result.Message | Should Be 'Outsystems platform server already installed with an higher version 11.23.0.2'
            }
            It 'Should output an error' { $err[-1] | Should Be 'Outsystems platform server already installed with an higher version 11.23.0.2' }
            It 'Should not throw' { { Install-OSServer -Version '11.23.1.0' -ErrorAction SilentlyContinue } | Should Not throw }
        }
        ...
        Context 'When the platform server is already installed with the right version but -Force switch is specified' {
            ...
            $result = Install-OSServer -Version '11.23.0.0' -Force -ErrorVariable err -ErrorAction SilentlyContinue
        ...
        Context 'When the platform server is not installed and -FullPathInstallDir is specified' {
            ...
            $result = Install-OSServer -Version '11.23.0.0' -InstallDir 'C:\Program Files\Outsystems' -FullPathInstallDir -ErrorVariable err -ErrorAction SilentlyContinue
        ...
        Context 'When the platform server is not installed and -AdditionalParameters is specified' {
            ...
            $result = Install-OSServer -Version '11.23.0.0' -InstallDir 'C:\Program Files\Outsystems' -AdditionalParameters "TestParam" -FullPathInstallDir -ErrorVariable err -ErrorAction SilentlyContinue
        ...
        Context 'When the caller changes the ErrorAction to stop' {
            ...
            Mock IsAdmin { return $false }
            It 'Should throw an exception' { { Install-OSServer -Version '11.23.0.0' -ErrorAction Stop } | Should throw }
        }
        ...
        Context 'When lifetime switch is specified' {
            ...
            $assRunParams = @{ 'CommandName' = 'Start-Process'; 'Times' = 1; 'Exactly' = $true; 'Scope' = 'Context'; 'ParameterFilter' = { $FilePath -eq "$ENV:TEMP\LifeTimeWithPlatformServer-11.19.1.0.exe" } }

            $null = Install-OSServer -Version '11.19.1.0' -WithLifeTime -ErrorVariable err -ErrorAction SilentlyContinue
